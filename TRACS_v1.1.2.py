...
class NewExpSettings(tk.Frame):
    ...
    def __init__(self, parent, controller):
        ...
        lbl_fdr = Label(fr_6, text="False discovery rate (e.g. 0.05 for 5%)", width="40", font=controller.label_font)
        lbl_fdr.pack(side=LEFT, padx=5, pady=5)
        self.txt_fdr = Entry(fr_6)
        self.txt_fdr.pack(fill=X, padx=5, pady=5, expand=True)

        # number of CPU cores to use
        fr_7 = Frame(fr_main_1, relief=controller.frame_relief, borderwidth=controller.frame_borderwidth)
        fr_7.pack(fill=X, padx=5, pady=5, anchor=N, expand=True)
        lbl_cores = Label(fr_7, text="Number of CPU cores to use", width="40", font=controller.label_font)
