...
class NewExpSettings(tk.Frame):
    ...
    def __init__(self, parent, controller):
        ...
        # dropdown box
        #self.opt_repnaming = ttk.Combobox(fr_6, state="readonly",  width="8", values=["-1, -2, -3", "_1, _2, _3", "-A, -B, -C", "_A, _B, _C"])
...
class TrimmingReads(tk.Frame):
    ...
    # function to print stuff to status box in window frame
    ...
    def do_trimming_v2(self, file_list, condition_name, cas9_type):
        ...
        for i in file_list:
            ...
            cmd_args = "-e 0.1 --cores 128 --trimmed-only"
    ...
    def do_bowtie2_alignment(self, file_list, condition_name, cas9_type):
        ...
        for i in file_list:
            ...
            if condition_name == "Library":
                ...
                # get replicate name
                next_char = self.controller.get_rep_char(file_list.index(i))
                input_file = os.path.join(trimmed_read_path, condition_name + "-" + next_char + cas9_type + global_vars.FILE_FLAGS['Trimmed read file'])
            ...
            cmd_suffix = " --norc -N 1 -p 128 | samtools view -bS - > %s" % cmd_f_out
    ...
    def do_mageck_count(self, file_list_1, file_list_2, condition_name_1, condition_name_2):
        ...
        for j in global_vars.CAS9_TYPE_NAMING:
            ...
            for i in file_list_1:
                ...
                input_file = os.path.join(aligned_read_path, condition_name_1 + "-" + next_char + j + global_vars.FILE_FLAGS['Aligned bam file'])

                cmd_sample_list = cmd_sample_list + condition_name_1 + "-" + next_char + j + ","
            ...
            for i in file_list_2:
                ...
                input_file = os.path.join(aligned_read_path, condition_name_2 + "-" + next_char + j + global_vars.FILE_FLAGS['Aligned bam file'])

                cmd_sample_list = cmd_sample_list + condition_name_2 + "-" + next_char + j + ","
...
class AnalyzeCounts(tk.Frame):
    ...
    def process_reads(self):
        ...
        #readsfile = "iOVCA147 Azure readcounts.txt" #### MOD TO RUN FIXED READ COUNT FILE ####
    ...
    def log2FC_calculator(self, name):
        ...
        # copy correct columns (denominator) into temp dataframe to mean function can be used across columns
        for col in range(0, int(self.num_replicates)):
